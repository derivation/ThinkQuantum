...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}2}]:} \PY{n}{filename} \PY{o}{=} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{./schrodin\PYZus{}yang.png}\PY{l+s+s1}{\PYZsq{}}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}2}]:} <matplotlib.image.AxesImage at 0x11a75aa90>
\end{Verbatim}
    ...
    If necessary, shrink the image to make the rest of the demonstration run
faster.

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}3}]:} \PY{k+kn}{from} \PY{n+nn}{skimage}\PY{n+nn}{.}\PY{n+nn}{transform} \PY{k}{import} \PY{n}{resize}
\end{Verbatim}


    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}4}]:} \PY{n}{n\PYZus{}pixels} \PY{o}{=} \PY{l+m+mi}{2}\PY{o}{*}\PY{o}{*}\PY{l+m+mi}{5}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}4}]:} <matplotlib.image.AxesImage at 0x81c0fcba8>
\end{Verbatim}
    ...
    \hypertarget{representing-the-image-as-a-state-vector}{%
\subsection{Representing the Image as a State
Vector}\label{representing-the-image-as-a-state-vector}}

    Represent the image as a quantum state as follows

\begin{figure}[h]
\centering
\includegraphics{./quantum_encoding.png}
\end{figure}

    The image begins as a matrix of values at certain pixel positions.
Unravel the matrices to form vectors. Then translate the pixel positions
...
or in general, \( \textbar{}\textrm{image}\rangle = \sum_k \alpha_k \textbar{}k\rangle \), where the basis state \(|k\rangle\) encodes the
position of each pixel and the amplitude \(\alpha_k\) encodes the pixel
value.

    Get the first color channel of the image and \texttt{ravel} it to form a
1-D vector.

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}5}]:} \PY{n}{data} \PY{o}{=} \PY{n}{im}\PY{p}{[}\PY{p}{:}\PY{p}{,}\PY{p}{:}\PY{p}{,}\PY{l+m+mi}{0}\PY{p}{]}\PY{o}{.}\PY{n}{ravel}\PY{p}{(}\PY{p}{)}
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}6}]:} \PY{n}{n\PYZus{}qubits}   \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{int\PYZus{}}\PY{p}{(}\PY{n}{np}\PY{o}{.}\PY{n}{log2}\PY{p}{(}\PY{n+nb}{len}\PY{p}{(}\PY{n}{data}\PY{p}{)}\PY{p}{)}\PY{p}{)}
...
image, \(|\textrm{image}\rangle\).
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}7}]:} \PY{n}{circ} \PY{o}{=} \PY{n}{init\PYZus{}state}\PY{o}{.}\PY{n}{construct\PYZus{}circuit}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{circuit}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
        \PY{n}{qr}   \PY{o}{=} \PY{n}{circ}\PY{o}{.}\PY{n}{qregs}
        \PY{c+c1}{\PYZsh{}circ.draw()}
\end{Verbatim}


    \hypertarget{how-the-quantum-hadamard-edge-detection-algorithm-works}{%
\subsection{How the Quantum Hadamard Edge Detection Algorithm
Works}\label{how-the-quantum-hadamard-edge-detection-algorithm-works}}
...
Consider a four pixel image and what happens when a Hadamard gate is
applied to the last qubit. We'll subscript the Hadamard operator to tell
which qubit it's acting on.
...
The terms with the difference of neighboring pixel values tell us
information about the image's edges. Here's why. Consider the following
row of pixels called \(p\) and the difference of its neighboring pixels
called \(\Delta p\):

\[p = [0, 0, 0, 1, 1, 1, 0, 0, 0]\]

\[\Delta p = [p(1)-p(0), p(2)-p(1), \ldots] = [0,0,1,0,0,-1,0,0]\]

The neighboring differences \(\Delta p\) take on non-zero values where
there are changes (edges) in the original image \(p\).

If we measure the state of the first qubit, and the result is \(1\),
then the state will have collapsed into the final state proportional to
...
state vector representation, apply the Hadamard gate to the first qubit.

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}8}]:} \PY{n}{circ}\PY{o}{.}\PY{n}{h}\PY{p}{(}\PY{n}{qr}\PY{p}{[}\PY{l+m+mi}{0}\PY{p}{]}\PY{p}{[}\PY{l+m+mi}{0}\PY{p}{]}\PY{p}{)}
\end{Verbatim}


\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}8}]:} <qiskit.extensions.standard.h.HGate at 0x1c21a15048>
\end{Verbatim}
...
the resulting state vector.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}9}]:} \PY{k+kn}{from} \PY{n+nn}{qiskit} \PY{k}{import} \PY{n}{BasicAer}\PY{p}{,} \PY{n}{execute}
\end{Verbatim}


    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}10}]:} \PY{n}{simulator} \PY{o}{=} \PY{n}{BasicAer}\PY{o}{.}\PY{n}{get\PYZus{}backend}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{statevector\PYZus{}simulator}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
         \PY{n}{result} \PY{o}{=} \PY{n}{execute}\PY{p}{(}\PY{n}{circ}\PY{p}{,} \PY{n}{simulator}\PY{p}{)}\PY{o}{.}\PY{n}{result}\PY{p}{(}\PY{p}{)}
         \PY{n}{final\PYZus{}state\PYZus{}vector} \PY{o}{=} \PY{n}{result}\PY{o}{.}\PY{n}{get\PYZus{}statevector}\PY{p}{(}\PY{n}{circ}\PY{p}{)}
\end{Verbatim}


    \hypertarget{decode-the-state-vector-back-into-an-image}{%
\subsection{Decode the State Vector Back into an
Image}\label{decode-the-state-vector-back-into-an-image}}

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}11}]:} \PY{n}{edge} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{real}\PY{p}{(}\PY{n}{final\PYZus{}state\PYZus{}vector}\PY{p}{)}
         \PY{n}{n\PYZus{}rows}    \PY{o}{=} \PY{n+nb}{int}\PY{p}{(}\PY{n}{np}\PY{o}{.}\PY{n}{sqrt}\PY{p}{(}\PY{n+nb}{len}\PY{p}{(}\PY{n}{edge}\PY{p}{)}\PY{p}{)}\PY{p}{)}
         \PY{n}{edge} \PY{o}{=} \PY{n}{edge}\PY{o}{.}\PY{n}{reshape}\PY{p}{(}\PY{n}{n\PYZus{}rows}\PY{p}{,} \PY{o}{\PYZhy{}}\PY{l+m+mi}{1}\PY{p}{)}
\end{Verbatim}
...
columns, zero out the odd columns.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}12}]:} \PY{n}{edge}\PY{p}{[}\PY{p}{:}\PY{p}{,}\PY{p}{:}\PY{p}{:}\PY{l+m+mi}{2}\PY{p}{]} \PY{o}{=} \PY{l+m+mi}{0}
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}13}]:} \PY{n}{fig}\PY{p}{,} \PY{n}{ax} \PY{o}{=} \PY{n}{plt}\PY{o}{.}\PY{n}{subplots}\PY{p}{(}\PY{l+m+mi}{1}\PY{p}{,}\PY{l+m+mi}{2}\PY{p}{)}
